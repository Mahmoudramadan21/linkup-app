...
const ProfileHeaderSkeleton: React.FC = memo(() => {
  return (
    <div
      className={`${styles['profile-header__container']} animate-pulse`}
      role="region"
    ...
    >
      ...
      <div className={styles['profile-header__header']}>
        <div className={styles['profile-header__cover']}>
          ...
          <div className={`${styles['profile-header__cover-placeholder']} bg-neutral-gray`} />
          
          {/* Edit Cover Button */}
          <div className={`${styles['profile-header__edit-btn']} bg-neutral-gray rounded-full w-28 h-10`} />
        </div>

        {/* Profile Picture */}
        <div className={styles['profile-header__info']}>
          <div className={styles['profile-header__profile-pic']}>
            <div className={`${styles['profile-header__profile-placeholder']} bg-neutral-gray`} />
          </div>
        </div>
      </div>
      ...
      <div className={styles['profile-header__content']}>
        ...
        <div className={styles['profile-header__stats']}>
          {Array.from({ length: 3 }, (_, i) => (
            <div key={i} className={styles['profile-header__stat__item']}>
              <div className="h-7 w-16 bg-neutral-gray rounded mx-auto" />
              <div className="h-4 w-20 bg-neutral-gray rounded mx-auto mt-2" />
            </div>
          ))}
        </div>
        ...
        <div className={styles['profile-header__actions']}>
          <div className="h-11 w-32 bg-neutral-gray rounded-full" />
          <div className="h-11 w-11 bg-neutral-gray rounded-full" />
        </div>
      </div>
      ...
      <div className={styles['profile-header__highlights']}>
        {Array.from({ length: 6 }, (_, i) => (
          <div key={i} className={styles['profile-header__highlight-item']}>
            <div className="w-16 h-16 bg-neutral-gray rounded-full ring-4 ring-[var(--section-bg)]" />
            <div className="h-4 w-16 bg-neutral-gray rounded mt-2 mx-auto" />
          </div>
        ))}
      </div>
      ...
      <div className={styles['profile-header__tabs']}>
        {['Posts', 'Saved'].map((_, i) => (
          <div key={i} className={styles['profile-header__tab']}>
            <div className="w-8 h-8 bg-neutral-gray rounded mx-auto" />
            <div className="h-4 w-14 bg-neutral-gray rounded mt-2 mx-auto" />
          </div>
        ))}
      </div>
    </div>
